...
try {
    ...
    Write-Output "Node.js is already installed: $(node --version), npm $(npm --version)"
} catch {
    Write-Output 'Node.js not found, installing...'
    try {
        # Try to use nvm-windows if available
        $nvmPath = "$env:ProgramFiles\nvm\nvm.exe"
        if (Test-Path $nvmPath) {
            Write-Output "nvm-windows found, using it to install Node.js LTS..."
            & $nvmPath install lts
            & $nvmPath use lts
            $env:Path = [System.Environment]::GetEnvironmentVariable('Path','Machine') + ';' + [System.Environment]::GetEnvironmentVariable('Path','User')
        } else {
            # Install Node.js directly using official installer
            Write-Output "Installing Node.js LTS via official installer..."
            ...
            if ($process.ExitCode -ne 0) {
                Write-Output "Warning: Node.js installer exited with code $($process.ExitCode)"
            }
        ...
        try {
            node --version | Out-Null
            npm --version | Out-Null
            Write-Output "Node.js installed successfully: $(node --version), npm $(npm --version)"
        } catch {
            Write-Output "Warning: Node.js may not be available in current session, please restart PowerShell"
        }
    } catch {
...
try {
    ...
    node --version | Out-Null
    npm --version | Out-Null
    Write-Output "Node.js and npm are available: $(node --version), npm $(npm --version)"
} catch {
    Write-Output "Warning: Node.js or npm may not be available in current session"
    Write-Output "Please restart PowerShell or manually add Node.js to PATH"
}
